...
import type { CommercialApiKeyConfig, Config } from '@/types/config';
...
const normalizeCommercialApiKey = (entry: unknown): CommercialApiKeyConfig | null => {
  if (entry === undefined || entry === null) return null;
  ...
  const apiKey = entry['api-key'] ?? entry.apiKey ?? entry.key;
  const trimmed = String(apiKey || '').trim();
  if (!trimmed) return null;

  const allowedModels = entry['allowed-models'] ?? entry.allowedModels ?? entry['allowed_models'];
  const usage = isRecord(entry.usage) ? entry.usage : null;
  return {
    apiKey: trimmed,
    ...
    allowedModels: Array.isArray(allowedModels)
      ? allowedModels.map((value) => String(value ?? '').trim()).filter(Boolean)
      : [],
    usage: usage
      ? {
          todayRequests: Number(usage['today_requests'] ?? usage.todayRequests ?? 0),
          todayTokens: Number(usage['today_tokens'] ?? usage.todayTokens ?? 0),
          totalRequests: Number(usage['total_requests'] ?? usage.totalRequests ?? 0),
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  if (Array.isArray(apiKeysRaw)) {
    config.apiKeys = apiKeysRaw
      .map((entry) => normalizeCommercialApiKey(entry))
      .filter(Boolean) as CommercialApiKeyConfig[];
  }
