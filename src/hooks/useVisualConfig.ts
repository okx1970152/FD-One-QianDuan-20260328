...
function makeVisualApiKeyEntry(partial?: Partial<VisualApiKeyEntry>): VisualApiKeyEntry {
  return {
    ...
    note: partial?.note || '',
    allowedModels: Array.isArray(partial?.allowedModels) ? partial!.allowedModels.filter(Boolean) : [],
  };
}
...
function parseApiKeyEntries(raw: unknown): VisualApiKeyEntry[] {
  ...
  for (const item of raw) {
    if (typeof item === 'string') {
      ...
      if (!apiKey) continue;
      entries.push(makeVisualApiKeyEntry({ apiKey }));
      continue;
    }

    const record = asRecord(item);
    if (!record) continue;
    const rawApiKey = record['api-key'] ?? record.apiKey ?? record.key ?? record.Key;
      ...
      makeVisualApiKeyEntry({
        ...
        expiresAt:
          typeof (record['expires-at'] ?? record.expiresAt) === 'string'
            ? String(record['expires-at'] ?? record.expiresAt).trim()
            : '',
        createdAt:
          typeof (record['created-at'] ?? record.createdAt) === 'string'
            ? String(record['created-at'] ?? record.createdAt).trim()
            : '',
        enabled:
          typeof (record.enabled ?? record.Enabled) === 'boolean'
  ...
  const loadVisualValuesFromYaml = useCallback((yamlContent: string) => {
    try {
      ...
      const newValues: VisualConfigValues = {
        ...
        apiKeys: resolveApiKeyEntries(parsed),
      ...
      try {
        ...
        const apiKeys = values.apiKeys
          .map((entry) => ({
            ...
            'allowed-models': entry.allowedModels.map((model) => model.trim()).filter(Boolean),
          }))
